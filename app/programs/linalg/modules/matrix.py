...
def parse_one(params):
    matrix = {}
    t = params['matrix'].split('{', maxsplit=1)[1].replace('{', '').replace('}', '').split(',')
    b = []
    for i in t:
...
def determ(*oper,**matrix):
    ...
    return sp.latex(sp.Rational(matrix[keys[0]].det())), 'det'+sp.latex(matrix[keys[0]])+'='
...
def eigenval(*oper,**matrix):
    keys = list(matrix.keys())
    return sp.latex(matrix[keys[0]].eigenvals()), 'eigenvalues'+sp.latex(matrix[keys[0]])+'='

@result
def eigenvec(*oper,**matrix):
    keys = list(matrix.keys())
    return sp.latex(matrix[keys[0]].eigenvects()), 'eigenvectors'+sp.latex(matrix[keys[0]])+'='
